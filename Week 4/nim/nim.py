...
class NimAI():
    ...
    def best_future_reward(self, state):
        ...
        # no available actions given the state
    ...
    def best_action(self, state, available_actions):
        """
        Get the best action given the available actions
        """
        best = list(available_actions)[0]

        for action in available_actions:
            if self.get_q_value(state, action) > self.get_q_value(state, best) :
                best = action
        return best
    ...
    def choose_action(self, state, epsilon=True):
        ...
        if epsilon:
            num = random.uniform(0,1) # random float from 0-1 inclusive
            if num <= self.epsilon:
